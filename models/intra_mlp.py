...
def index_points(device, points, idx):
    ...
    repeat_shape[0] = 1
    batch_indices = (
        torch.arange(B, dtype=torch.long).cuda().view(view_shape).repeat(repeat_shape)
    )
...
def knn_l2(device, net, k, u):
    ...
    def u_block(batch_size, npoint, u):
        ...
        n = npoint // u
        for i in range(n):
            block[:, (i * u) : (i * u + u), (i * u) : (i * u + u)] = numpy.ones(
                [batch_size, u, u]
            ) * (-INF)
        return block
    ...
    minus_distance = (
        ...
        + torch.Tensor(u_block(batch_size, npoint, u)).cuda()
    )
